...
node {
    try {
        ...
        def imageName = "nxt-admin-app-develop-i"
        def containerName = "nxt-admin-app-develop-c"
        def tag = "1.0"

        stage('SCM Checkout') {
            git branch: 'develop', credentialsId: '22f48ea6-da0e-4196-8df4-393c3bea75e1', url: 'http://10.5.0.5:3000/next/nxt-admin-app.git'
        }
        stage('Build') {
            sh "npm install"
        }
        ...
        stage('Run new container') {
            sh "docker run -d --name ${containerName} -p 3003:80 -e HOST_ENV=dev ${imageName}:${tag}"
        }
    } catch (e) {
