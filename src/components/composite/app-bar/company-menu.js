...
export default function CompanyMenu({ company, branches, handleCompanyMenuChange }) {
            ...
            <ButtonGroup variant="contained" color="primary" ref={anchorRef} aria-label="split button">
                {appContext.length > 0 &&
                    <Button
                        ...
                        {appContext[localStorage.getItem("selectedIndex") ? localStorage.getItem("selectedIndex") : 0].name}
                    </Button>}
                <Box pt={1}
                    style={{ "color": "#f18e00" }}
                    color="primary"
                                ...
                                <MenuList id="split-button-menu">
                                    {appContext.map((item, index) => (
                                        <>
                                            {index === 0 &&
                                                <>
                                                    <Box ml={2} fontSize={14} fontWeight={400}>
                                                        Company
                                                    </Box>
                                            ...
                                            {index === 1 &&
                                                <>
                                                    <Divider></Divider>
                                                    <Box ml={2} mt={2} fontSize={14} fontWeight={400}>
                                                        Stores
                                                    </Box>
                                                </>}
                                            {index >= 1 &&
